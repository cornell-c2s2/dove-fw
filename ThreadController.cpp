...
ThreadController::ThreadController(unsigned long _interval): MyThread(){
	cached_size = 0;
...
*/
bool ThreadController::add(MyThread* _thread){
...
void ThreadController::remove(MyThread* _thread){
...
MyThread* ThreadController::get(int index){
	int pos = -1;
	for(int i = 0; i < MAX_THREADS; i++){
		if(thread[i] != NULL){
			pos++;
